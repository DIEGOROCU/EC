...
void ShowBuffer (int buffer[]) {
    ...
    LCDMEM[9]  = alphabetBig[(buffer[0])-65][0]; LCDMEM[10] = alphabetBig[(buffer[0])-65][1]; // Posición 1 (Izquierda)
    LCDMEM[5]  = alphabetBig[(buffer[1])-65][0]; LCDMEM[6]  = alphabetBig[(buffer[1])-65][1]; // Posición 2
    ...
    switch(__even_in_range(UCA1IV, USCI_UART_UCTXCPTIFG)) {
        ...
        case USCI_UART_UCRXIFG: {     // EVENTO: Ha llegado un carácter desde el PC
            ...
            if (received_char >= 'A' && received_char <= 'Z') {
                int i;
                // Desplaza el buffer a la izquierda (el índice 1 pasa al 0, etc.)
                for(i = 0; i < 5; i++) {
                    buffer[i] = buffer[i+1];
                }
                // La letra recién tecleada se guarda en el hueco 5 (que ahora se dibuja a la derecha)
