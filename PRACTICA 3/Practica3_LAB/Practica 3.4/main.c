...
int main(void) {
  ...
  // Configuracion TIMER_B (inicialmente apagado)
  TB0CTL = TASSEL__ACLK | ID__1 | MC__STOP | TBCLR;
  TB0CCR0 = 20000;
  TB0CCTL0 = CCIE;
  ...
  P1OUT &= ~BIT0;
  // VERDE
  P9DIR |= BIT7; // del puerto 1 activo el bit 0
  P9OUT &= ~BIT7;
...
// INTERRUPCIONES PARA EL TIMER B (parpadeo de los leds en 3s)
#pragma vector = TIMER0_B0_VECTOR
__interrupt void TIMER0_B0_ISR(void) {
  display_num_lcd(n);
  // Primeros parpadeos
  if (parpadeos < 6) {
    // Apaga o enciende segun cuantos 0,5s hayan pasado (parpadeo++ ocurre cada 0,5s, en unos se encienden y en otros apagan)
    if (parpadeos % 2) {
      P1OUT |= BIT0;
      P9OUT |= BIT7;
    }
  ...
  // Tras parpadear las 5 veces (3 encendidos y 2 apagados) apagamos la 3 vez, reiniciamos el contador y el numero de parpadeos para el proximo reinicio
  else {
  ...
  TB0CCTL0 &= ~CCIFG; // Quitar la flag de interrupciones
}
...
__interrupt void Port_1(void) {
  ...
  // Si se resetea activamos el timer B
  else if (P1IFG & BIT2) {
    TB0CTL = TB0CTL | MC__UP;
